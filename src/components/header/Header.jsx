		...
		<>
			<header className={`${!loc ? "header" : "header__greeting"}`}>
				<button
					className={`logo__btn ${
						!loc
							? "logo__btn_place_header"
							: "logo__btn_place_greeting"
					}`}
					onClick={navToMain}>
				...
				{btnDropList && !modalState && login && !loc && (
					<img
						src={dropListIcon}
						alt='иконка с тремя полосками'
						onClick={handleNavbar}
						className='header__droplist'></img>
				)}
				...
				{!btnDropList && login && !loc && (
					<button
						className={`header__btn header__btn-acc ${btnAccStyle}`}>
						<img
							src={profile}
							onClick={navToAcc}
							className='header__btn header__btn-auth'
							alt='Информация аккаунта'></img>
					</button>
				)}
				{loc && (
					<h1 className='header__greeting-text'>{greetingText}</h1>
				)}
			</header>
			{modalState && (
				<NavigatePopup
